import os
from solvers.dlts import tree_search
import logging
from solvers.dlts.solution_cpmp import solution_cpmp
import sys
import glob

# TODO in some places the logger is used, here we have print. Consistency?

def parse_unlabeled_benchmark(fp):
    lines = [l.strip() for l in fp.readlines() if l.strip()]
    if not lines: return 0, 0, [], [], [], 0.0
    
    header = lines[0].split()
    stacks = int(header[0])
    total_contenedores = int(header[1])
    
    # Aplicando tu regla de altura máxima
    tiers = int((total_contenedores / stacks) + 2)
    
    start_state = []
    # Leer bloque por bloque y rellenar con ceros (slots vacíos)
    for i in range(1, stacks + 1):
        datos = list(map(int, lines[i].split()))
        n_cajas = datos[0]
        cajas = datos[1:]
        
        pad_len = tiers - len(cajas)
        start_state.extend(cajas)
        start_state.extend([0] * pad_len)
        
    rr = [start_state]
    return stacks, tiers, rr, [], [], 0.0
...
def solve_instances(branch_network, ddir, output_path, upper_bound, ts_param_p, ts_param_d, ts_param_d2, ts_param_n,
    ...
    if val_insts:
        ...
        insts = [f for f in glob.glob("{0}/*".format(ddir)) if os.path.isfile(f)]
    ...
    for ff in sorted(insts):
        ...
        with open(inst_joined, 'r') as fp:
            stacks, tiers, rr, ll, ll_v, ref_rt = parse_unlabeled_benchmark(fp)
        if len(rr) < 1:
            continue
        cells = rr[0]
...
#         rt = time.time() - start_time_test
        ...
        if solutionMoveCount == 0:
            ...
            solved += 1
            gap = (solutionMoveCount - refMoveCount) / refMoveCount if refMoveCount > 0 else 0.0
            if verbose >= 1:
...
#             #Write solution to disk TODO change this to be CLI output
    ...
    if verbose >= 1:
        print("{0}% of {1} instances were solved!".format(round(solved / (solved + unsolved) * 100, 2) if (solved+unsolved) > 0 else 0, solved + unsolved))
        print("Average number of moves of dnn: {0}".format(round(moves_sum / solved, 2) if solved > 0 else 0))
        if ref_moves_sum > 0:
            print("Average number of moves of reference solution: {0}".format(round(ref_moves_sum / (solved+unsolved), 2)))
        else:
            print("Average number of moves of reference solution: N/A")
    elif tuning:
        print(solved)
        print(moves_sum)
